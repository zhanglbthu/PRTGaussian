...
def training(dataset, opt, pipe, testing_iterations, debug_from, 
    ...
    if not debug:
        ...
        for iteration in range(first_iter, opt.iterations + 1):        
            ...
            light_coeffs = get_sh_coeffs(direction=(viewpoint_cam.light_phi, viewpoint_cam.light_theta), order=order)
            
            gt_image = viewpoint_cam.original_image.cuda()

            diffuse_colors = compute_diffuse_colors(light_coeffs, pixels, decoder, order)
            ...
            if iteration % 1000 == 0:
                ...
                save_image(sh_map_corrected, os.path.join(sh_map_path, '{0:05d}'.format(iteration) + ".png"))
            # endregion
            ...
            with torch.no_grad():
                ...
                # Log and save
                training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, pixels,
                                decoder=decoder, order=order)
    ...
    render_set(dataset.model_path, "train", opt.iterations, new_scene.getTrainCameras(), pixels, decoder, order=order)
    render_set(dataset.model_path, "test", opt.iterations, new_scene.getTestCameras(), pixels, decoder, order=order)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, pixels, decoder=None, order=3):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    light_coeffs = get_sh_coeffs(direction=(viewpoint.light_phi, viewpoint.light_theta), order=order)
                    diffuse_colors = compute_diffuse_colors(light_coeffs, pixels, decoder, order)
                    image = diffuse_colors.view(RESOLUTION[1], RESOLUTION[0], 3).permute(2, 0, 1)
...
def render_set(model_path, name, iteration, views, pixels, decoder = None, order=3):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        light_coeffs = get_sh_coeffs(direction=(view.light_phi, view.light_theta), order=order)
