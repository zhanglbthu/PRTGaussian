...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda() # * (N, 3, 16=(max_sh_degree+1)^2)
        ...
        print("fused_color.shape: ", fused_color.shape)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        n_init_points = self.get_xyz.shape[0] # * 获取初始点数
        # Extract points that satisfy the gradient condition
        padded_grad = torch.zeros((n_init_points), device="cuda") # * 创造一个全零张量
        padded_grad[:grads.shape[0]] = grads.squeeze() # * 赋值
